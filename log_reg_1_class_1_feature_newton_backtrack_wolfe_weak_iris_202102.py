...
if len(sys.argv)!=9:
    print("Use: log_reg_1_class_1_feature_newton_backtrack_iris_202102.py <features> <target> <#iterations newton> <absolute tolerance> <lift coeff c1> <coeff c2> <gamma c> <#iterations bt>",file=sys.stderr)
    sys.exit(1)
...
def bt(w,d):
    ...
    while n<niter_bt:
        ...
        if fcost(w+s*d)<=fcost(w)+c1*fgrad(w).T @ (s*d):
            if c2*fgrad(w).T @ d < fgrad(w+s*d).T @ d:  
                print("break" )
                break
        n=n+1
    print("n_bt=",n)
